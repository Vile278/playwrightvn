import { test, expect, Page } from "@playwright/test"; // import các thư viện cần thiết từ Playwright
import {RegisterAction} from "../actions/register.action"; // import các hàm thao tác với page từ action class
import { RegisterPage } from "../pages/register.page"; // import các element locators để verify
import  inputData  from "../data/inputData.json"; // import data test từ file json để áp dụng data driven test
...
test.describe ('Submit', () => {
    test('@submit Home TC1: submit form ', async ({ page }) => {
...
test.describe('Check elements', () => {
    test ('@check TC2: check radio buttons Male and Female', async ({ page }) => { 
        // const registerAction = new RegisterAction(page);
    ...
    test ('@check TC3: check tooltip text', async ({ page }) => {
        // const registerAction = new RegisterAction(page);
        // const registerPage = new RegisterPage(page);    
        // await registerAction.gotoRegisterPage();
    ...
    test ('check TC 4: check common elements on the Register page', async ({ page }) => {
    ...
    test ('check TC 5: check date of birth dropdown', async ({ page }) => {
